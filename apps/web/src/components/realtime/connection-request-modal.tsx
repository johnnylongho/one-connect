...
import { Sparkles, UserCheck, ShieldCheck, X, Check, BellRing, Phone, Building2, MessageSquare } from 'lucide-react';
    ...
    <div className="fixed inset-x-4 top-6 md:top-8 md:right-8 md:left-auto md:max-w-md z-50 animate-in fade-in slide-in-from-top-6 duration-300">
      <div className="relative overflow-hidden rounded-2xl bg-gradient-to-b from-slate-900 via-slate-900/95 to-slate-950 border border-cyan-500/40 p-5 shadow-2xl shadow-cyan-950/60 backdrop-blur-2xl text-white ring-1 ring-white/10">
        ...
        <div className="flex items-center gap-3.5 my-3">
          <img
            src={
              incoming.requesterAvatar && !incoming.requesterAvatar.includes('avatar-johnny-long.jpg')
                ? incoming.requesterAvatar
                : `https://ui-avatars.com/api/?name=${encodeURIComponent(incoming.requesterName.split('(')[0]?.trim() || 'Khach')}&background=0284c7&color=fff&bold=true`
            }
            alt={incoming.requesterName}
            className="w-13 h-13 rounded-2xl object-cover border-2 border-cyan-400/50 shadow-md shrink-0 ring-2 ring-cyan-500/20"
          />
          <div className="min-w-0 flex-1">
            <h4 className="text-base font-black text-white tracking-tight truncate">
              {incoming.requesterName}
            </h4>
            {incoming.requesterCompany && (
              <p className="text-xs font-semibold text-cyan-300 truncate flex items-center gap-1.5 mt-0.5">
                <Building2 className="w-3.5 h-3.5 text-cyan-400 shrink-0" />
                <span>{incoming.requesterCompany}</span>
              </p>
            )}
            {incoming.requesterPhone && (
              <p className="text-xs font-medium text-emerald-300 truncate flex items-center gap-1.5 mt-0.5">
                <Phone className="w-3.5 h-3.5 text-emerald-400 shrink-0" />
                <span>{incoming.requesterPhone}</span>
              </p>
            )}
          </div>
        </div>

        {incoming.requesterNote && (
          <div className="mb-3 bg-cyan-950/40 p-2.5 rounded-xl border border-cyan-500/20 text-xs text-cyan-100 flex items-start gap-2">
            <MessageSquare className="w-4 h-4 text-cyan-400 shrink-0 mt-0.5" />
            <div className="min-w-0 flex-1">
              <span className="font-bold text-cyan-300 block text-[11px] uppercase tracking-wider mb-0.5">Nhu cầu kết nối / Lời nhắn:</span>
              <p className="italic text-slate-200 line-clamp-2 leading-relaxed">&ldquo;{incoming.requesterNote}&rdquo;</p>
            </div>
          </div>
        )}

        <p className="text-[11.5px] text-slate-400 mb-4 bg-slate-950/50 p-2.5 rounded-xl border border-white/5 flex items-center gap-1.5 leading-relaxed">
          <ShieldCheck className="w-3.5 h-3.5 text-emerald-400 shrink-0" />
          <span>Thông tin được mã hóa bảo mật theo chuẩn Nghị định PDPL 13/2023/NĐ-CP.</span>
        </p>
